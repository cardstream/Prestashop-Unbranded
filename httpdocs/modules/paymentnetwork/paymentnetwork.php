 ...
 *
 *  @author    Support <support@paymentnetwork.com>
...
class PaymentNetwork extends PaymentModule {
	...
	const THREEDS_TWO_HOSTED = 'https://test.3ds-pit.com/hosted/';
	const THREEDS_TWO_DIRECT = 'https://test.3ds-pit.com/direct/';
	const HOSTED_URL_MODAL = 'https://gateway.example.com/hosted/modal';
	...
	public function __construct() {
		...
		$this->version     = '2.0.2';
		$this->author      = 'Payment Network';
		$this->controllers = array('payment', 'validation');
		$this->is_eu_compatible = 1;

		parent::__construct();

		$this->displayName            = 'Payment Network Gateway';
		$this->description            = $this->l('Process payments with Payment Network');
		$this->ps_versions_compliancyPAYMENT_NETWORK_INTEGRATION_TYPE = array('min' => '1.7', 'max' => _PS_VERSION_);


		$this->gateway_url = Configuration::get('PAYMENT_NETWORK_GATEWAY_URL');
		$type = Configuration::get('PAYMENT_NETWORK_INTEGRATION_TYPE');

		if (
			// Make sure we're given an valid URL
			!empty($this->gateway_url) &&
			preg_match('/(http[s]?:\/\/[a-z0-9\.]+(?:\/[a-z]+\/?)+)/i', $this->gateway_url) != false
		) {
		    ...
		    switch ($type) {
				case 'direct_v2':
					$this->gateway_url = self::THREEDS_TWO_DIRECT;
                    break;
                case 'direct':
                    $this->gateway_url = self::DIRECT_URL;
                    break;
                case 'hosted_v3':
                    $this->gateway_url = self::HOSTED_URL_MODAL;
                    break;
                case 'iframe_v2':
                case 'iframe':
                case 'hosted_v2':
                    $this->gateway_url = self::THREEDS_TWO_HOSTED;
                    break;
                case 'hosted':
                default:
                    $this->gateway_url = self::HOSTED_URL;
	...
	public function uninstall() {
		...
		return (
			...
			Configuration::deleteByName('PAYMENT_NETWORK_CURRENCY_ID') &&
			Configuration::deleteByName('PAYMENT_NETWORK_COUNTRY_ID') &&
			...
			Configuration::deleteByName('PAYMENT_NETWORK_CALLBACK') &&
...
//		}
	...
	private function generateDirectForm() {
        ...
        $in_array = in_array(Configuration::get('PAYMENT_NETWORK_INTEGRATION_TYPE'), ['hosted_v2', 'hosted_v3', 'iframe_v2', 'direct_v2'], true);
        $this->context->smarty->assign(
			array(
				'url' => $this->context->link->getModuleLink($this->name, 'payment', array(), true),
                'include_device_data'  => $in_array,
                'device_data'  => $deviceData,
            )
		);
        $str = $this->context->smarty->fetch('module:payment_network/views/templates/front/direct_payment.tpl');
        return $str;
	}
	...
	public function hookPaymentReturn($params) {
		...
		return $this->context->smarty->fetch('module:payment_network/views/templates/front/payment_confirmation.tpl');
	}
	...
	public function getContent() {
		...
		if (Tools::isSubmit('submit' . $this->name)) {
			...
			Configuration::updateValue('PAYMENT_NETWORK_CURRENCY_ID', Tools::getvalue('payment_network_currency_id'));
			Configuration::updateValue('PAYMENT_NETWORK_COUNTRY_ID', Tools::getvalue('payment_network_country_id'));
			...
			Configuration::updateValue('PAYMENT_NETWORK_CALLBACK', Tools::getvalue('payment_network_callback'));
	...
	public function displayForm() {
		...
		$fields_form[0]['form'] = array(
			'legend' => array(
				'title' => $this->l('Payment Network Settings'),
			),
			'input'  => array(
				array(
					'type'     => 'text',
				...
				array(
					...
					'options'  => array(
						'query' =>  array(
                            ...
                            array(
								'value' => 'hosted_v2',
								'label' => $this->l('Hosted 3-D Secure'),
							),
							array(
								'value' => 'hosted_v3',
							...
							array(
								'value' => 'direct_v2',
								'label' => $this->l('Direct 3-D Secure'),
							),
						),
						'id'    => 'value',
						'name'  => 'label',
				...
				array(
					...
					'name'     => 'payment_network_currency_id',
				...
				array(
					...
					'name'     => 'payment_network_country_id',
					'class'    => 'fixed-width-xs',
					'required' => true,
				),
				array(
		...
		$helper->fields_value['payment_network_currency_id']      = Configuration::get('PAYMENT_NETWORK_CURRENCY_ID');
		$helper->fields_value['payment_network_country_id']       = Configuration::get('PAYMENT_NETWORK_COUNTRY_ID');
		...
		$helper->fields_value['payment_network_callback']         = Configuration::get('PAYMENT_NETWORK_CALLBACK');
	...
	private function generatePaymentForm($order) {
		...
		$parameters = array(
			...
			'currencyCode'      => (
				is_numeric(Configuration::get('PAYMENT_NETWORK_CURRENCY_ID')) ?
				Configuration::get('PAYMENT_NETWORK_CURRENCY_ID') :
				$currency->iso_code_num
			),
			'countryCode'       => Configuration::get('PAYMENT_NETWORK_COUNTRY_ID'),
			'action'            => "SALE",
			'type'              => 1,
		...
		$includeDeviceData = in_array(Configuration::get('PAYMENT_NETWORK_INTEGRATION_TYPE'), ['hosted_v2', 'hosted_v3', 'iframe_v2', 'direct_v2'], true);
        if ($includeDeviceData) {
			include('hardwareinfo.html');
            $parameters = array_merge($parameters, [
				'threeDSRedirectURL' => $this->context->link->getModuleLink($this->name, 'validation', array(), true),
                'threeDSVersion'    => 2,
                ...
                'threeDSOptions[browserJavaEnabled]'    => $_COOKIE['java'],
                'threeDSOptions[browserLanguage]'		   => $_COOKIE['language'],
                'threeDSOptions[browserScreenColorDepth]'  => $_COOKIE['screen_depth'],
                'threeDSOptions[browserScreenHeight]'      => $_COOKIE['screen_height'],
                'threeDSOptions[browserScreenWidth]'       => $_COOKIE['screen_width'],
                'threeDSOptions[browserTimeZone]'          => $_COOKIE['timezone'],
	...
	public function generateDirectPaymentForm($order, $post) {
		...
		if (Configuration::get('PAYMENT_NETWORK_MERCHANT_PASSPHRASE')) {
     ...
     * @return array
     */
	public function getFormParameters($order) {
        ...
        if (Configuration::get('INTEGRATION_TYPE') == 'hosted_v2') {
            //$form['threeDSVersion'] = 2;
        }

		$form['redirectURL'] = $link;
		if (Configuration::get('PAYMENT_NETWORK_CALLBACK', 'Y') == 'Y') {
			$form['callbackURL'] = $link;
		}
		if (Configuration::get('PAYMENT_NETWORK_FORM_RESPONSIVE', 'Y') == 'Y') {
	...
	public function hookHeader() {
		...
		$this->context->controller->addCSS($this->_path . 'views/css/payment_network.css', 'all');
...
SCRIPT;
	...
	public function validatePayment($context, $data) {
		...
		if ($data['responseCode'] == 65802 && $data['threeDSVersion'] == 1) {
		...
		else if ($data['responseCode'] == 65802) {
			$_SESSION['threeDSRef'] = $data['threeDSRef'];
			$_SESSION['threeDSMethodData'] = $data['threeDSMethodData'];
			var_dump($data);
			die(include('3dsv2-form.php'));
		}

		if(isset($data['threeDSMethodData']) || $data['cres']) {
			$req = array(
				'merchantID' =>	Configuration::get('PAYMENT_NETWORK_MERCHANT_ID'),
			...
			$req['signature'] = $this->createSignature(
				$req,
				Configuration::get('PAYMENT_NETWORK_MERCHANT_PASSPHRASE')
			);
			$this->validatePayment($this->context, $this->makeRequest($this->gateway_url,$req));
		}
		...
		if ($data['responseCode'] == 0) {
			...
			$amount = Cart::getTotalCart($cart->id);
